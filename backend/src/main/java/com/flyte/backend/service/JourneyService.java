...
public class JourneyService {
    ...
    public Journey createJourney(CreateJourneyRequest journey, String userID) {
        ...
        Journey sameFlightJourney = journeyRepository.findByFlightNumber(journey.getFlightNumber()).stream().findFirst()
                .orElse(null);
        if (sameFlightJourney != null) {
            ...
            flightRoom.setName(
                    journey.getSource() + "-" + journey.getDestination() + "-" + journey.getFlightNumber());
            flightRoom.setType(RoomType.FLIGHT);
            Instant expiryTime = destinationSlotObj.getSlotEnd().plusSeconds(24 * 60 * 60); // 2 hours after arrival
                                                                                            // slot end
            ...
            String description = String.format(
                    ...
                    journey.getDepartureTime().toString(), // Formats to "7:30 PM on Sunday, October 5, 2025"
                    journey.getArrivalTime().toString()// Formats to "9:00 PM on Sunday, October 5, 2025"
                    , expiryTime.toString());
        ...
        Journey sameSourceSlotJourney = journeyRepository.findBySourceAndSourceSlot(journey.getSource(), sourceSlot)
